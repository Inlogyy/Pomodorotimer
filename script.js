...
function loadData() {
    ...
    if (savedIntervals) {
        ...
        workIntervalInput.value = intervals.work;
        shortBreakIntervalInput.value = intervals.shortBreak;
        longBreakIntervalInput.value = intervals.longBreak;
    }
}
...
function saveData() {
    ...
    localStorage.setItem('intervals', JSON.stringify({
        work: workIntervalInput.value,
        shortBreak: shortBreakIntervalInput.value,
        longBreak: longBreakIntervalInput.value
    }));
}
...
function updateTimer() {
    if (isActive && timeLeft > 0) {
        ...
        updateTimeDisplay();
        saveData(); // Save data after each session
    }
}
...
[todoTasks, inProgressTasks, doneTasks].forEach(column => {
    ...
    column.addEventListener('drop', (e) => {
        ...
        if (task) {
            ...
            renderKanbanBoard();
            updateProductivityScore();
            saveData(); // Save data after changing task status
        }
    });
});
